...
// (Fonte 8x8 Omitida por brevidade, assumindo que foi mantida em arquivo original ou adicionada aqui. 
// Vamos recolocar a matriz font8x8 basica para nao quebrar a compilacao).
static const uint8_t font8x8_basic[128][8] = {
    ...
    // Obs: Se faltar alguma letra do seu design original, o OS desenhará vazio.
};
...
// SUPER OTIMIZADO: Substituindo for loop lento por fast_memcpy
void gfx_swap_buffers(void) {
    if (!framebuffer || !back_buffer) return;
    fast_memcpy(framebuffer, back_buffer, total_pixels_64 * sizeof(uint32_t));
}

// SUPER OTIMIZADO: Substituindo loop for iterativo por fast_memset
void gfx_clear(uint32_t color) {
    // Memset padrao trabalha em bytes, mas como a cor é 32-bits, 
    // faremos um preenchimento em blocos otimizado.
...
// SUPER OTIMIZADO: Desenho Direto na memoria via Pointer Math (Sem ifs de limites por pixel)
void gfx_draw_rect(int x, int y, int w, int h, uint32_t color) {
    // Clamp Limits fora do Loop
    ...
    for (int i = start_y; i < end_y; i++) {
        ...
        for (int j = start_x; j < end_x; j++) {
            row_ptr[j] = color; // Escrita Direta O(1) sem Call Frame
        }
    }
}
...
void gfx_draw_cursor(int x, int y) {
    gfx_draw_rect(x, y, 4, 4, 0xFFFFFF); // Simples e veloz para testes de UI
}
