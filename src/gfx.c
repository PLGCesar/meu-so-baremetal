...
static void bga_write_register(uint16_t IndexValue, uint16_t DataValue) {
    outw(0x01CE, IndexValue);
    outw(0x01CF, DataValue);
}

void bga_set_video_mode(uint32_t w, uint32_t h, uint32_t bpp) {
    bga_write_register(4, 0);          // VBE_DISPI_DISABLED (0)
    bga_write_register(1, (uint16_t)w);   // VBE_DISPI_INDEX_XRES
    bga_write_register(2, (uint16_t)h);   // VBE_DISPI_INDEX_YRES
    bga_write_register(3, (uint16_t)bpp); // VBE_DISPI_INDEX_BPP
    bga_write_register(4, 0x41);       // VBE_DISPI_ENABLED | VBE_DISPI_LFB_ENABLED
}
...
static const uint8_t font8x8_basic[256][8] = {
    ...
    [192] = {0x18, 0x0C, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x00},
    [193] = {0x0C, 0x18, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x00},
    [194] = {0x18, 0x24, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x00},
    [195] = {0x34, 0x2C, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x00},
    [199] = {0x3C, 0x66, 0x60, 0x60, 0x3C, 0x18, 0x30, 0x00},
    [201] = {0x0C, 0x18, 0x7E, 0x60, 0x78, 0x60, 0x7E, 0x00},
    [202] = {0x18, 0x24, 0x7E, 0x60, 0x78, 0x60, 0x7E, 0x00},
    [205] = {0x0C, 0x18, 0x3C, 0x18, 0x18, 0x18, 0x3C, 0x00},
    [211] = {0x0C, 0x18, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00},
    [212] = {0x18, 0x24, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00},
    [213] = {0x34, 0x2C, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00},
    [218] = {0x0C, 0x18, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
    [224] = {0x18, 0x0C, 0x00, 0x3C, 0x02, 0x3E, 0x46, 0x3E},
    [225] = {0x0C, 0x18, 0x00, 0x3C, 0x02, 0x3E, 0x46, 0x3E},
    [226] = {0x18, 0x24, 0x00, 0x3C, 0x02, 0x3E, 0x46, 0x3E},
    [227] = {0x34, 0x2C, 0x00, 0x3C, 0x02, 0x3E, 0x46, 0x3E},
    [231] = {0x00, 0x00, 0x3C, 0x60, 0x60, 0x3C, 0x18, 0x30},
    [233] = {0x0C, 0x18, 0x00, 0x3C, 0x42, 0x7E, 0x40, 0x3C},
    [234] = {0x18, 0x24, 0x00, 0x3C, 0x42, 0x7E, 0x40, 0x3C},
    [237] = {0x0C, 0x18, 0x00, 0x18, 0x18, 0x18, 0x18, 0x18},
    [243] = {0x0C, 0x18, 0x00, 0x3C, 0x42, 0x42, 0x42, 0x3C},
    [244] = {0x18, 0x24, 0x00, 0x3C, 0x42, 0x42, 0x42, 0x3C},
    [245] = {0x34, 0x2C, 0x00, 0x3C, 0x42, 0x42, 0x42, 0x3C},
    [250] = {0x0C, 0x18, 0x00, 0x42, 0x42, 0x42, 0x42, 0x3E}
};
...
void gfx_init(multiboot_info_t* mbi) {
    ...
    if (mbi) {
        ...
        } else if ((mbi->flags & (1 << 11)) && mbi->vbe_mode_info != 0) {
    ...
    if (!framebuffer) {
        for (uint8_t slot = 0; slot < 32; slot++) {
            ...
            if ((id & 0xFFFF) == 0x1234 || (id & 0xFFFF) == 0x8086) {
...
void gfx_draw_cursor(int x, int y) {
    for (int cy = 0; cy < 16; cy++) {
        for (int cx = 0; cx < 12; cx++) {
            ...
            if (pixel == '*' || pixel == '.') gfx_put_pixel_alpha(x + cx + 3, y + cy + 3, 0x000000, 90);
        }
    }
    for (int cy = 0; cy < 16; cy++) {
        for (int cx = 0; cx < 12; cx++) {
            char pixel = cursor_sprite[cy][cx];
            if (pixel == '*') gfx_put_pixel(x + cx, y + cy, 0xFFFFFF);
            else if (pixel == '.') gfx_put_pixel(x + cx, y + cy, 0x181825);
        }
    }
}

void gfx_draw_landscape_sunset(int x, int y, int w, int h) {
    int horizon = y + (h * 6) / 10; int sun_cx = x + w / 2; int sun_cy = horizon - (h / 7);
    for (int py = y; py < y + h; py++) {
        int rel_y = py - y;
        for (int px = x; px < x + w; px++) {
            int rel_x = px - x; uint32_t color = 0;
            if (py < horizon) {
                ...
                if (t < 128) {
                    int t1 = t * 2;
                    r = 0x1A + ((0xE6 - 0x1A) * t1) / 255; g = 0x05 + ((0x39 - 0x05) * t1) / 255; b = 0x2E + ((0x46 - 0x2E) * t1) / 255;
                } else {
                    int t2 = (t - 128) * 2;
                    r = 0xE6 + ((0xFF - 0xE6) * t2) / 255; g = 0x39 + ((0xB7 - 0x39) * t2) / 255; b = 0x46 + ((0x03 - 0x46) * t2) / 255;
                }
                color = (r << 16) | (g << 8) | b;
                int dx = px - sun_cx; int dy = py - sun_cy; int dist2 = dx*dx + dy*dy;
                if (dist2 < 1200) color = 0xFFF1E6; else if (dist2 < 2800) color = 0xFFD166;
                if (py > horizon - 35 - (((rel_x * 9) % 40) + ((rel_x * 5) % 25))) color = 0x3A0CA3;
                if (py > horizon - 15 - (((rel_x * 13) % 30) + ((rel_x * 19) % 20))) color = 0x10002B;
            } else {
                int dx = px - sun_cx; if (dx < 0) dx = -dx;
                int ripple = (rel_x * 23 + rel_y * 11) % 17;
                if (dx < 80 && ripple < 9) color = 0xFFD166; else if (dx < 160 && ripple < 5) color = 0xF72585; else color = 0x050517;
            }
...
void gfx_draw_landscape_cosmos(int x, int y, int w, int h) {
    int cx = x + w / 3, cy = y + h / 2; int p_cx = x + (w * 3) / 4, p_cy = y + h / 3;
    for (int py = y; py < y + h; py++) {
        for (int px = x; px < x + w; px++) {
            uint32_t color = 0x03030D;
            int hash = (px * 73 + py * 137) % 197;
            if (hash == 7) color = 0xFFFFFF; else if (hash == 19) color = 0x4CC9F0; else if (hash == 42) color = 0xF72585;
            int dx = px - cx, dy = py - cy; int dist2 = dx*dx + dy*dy;
            if (dist2 < 16000) {
                int intensity = (16000 - dist2) * 180 / 16000;
                color += ((intensity) << 16) | (intensity / 4 << 8) | (intensity);
            }
            int pdx = px - p_cx, pdy = py - p_cy; int pdist2 = pdx*pdx + pdy*pdy;
            if (pdist2 < 1600) {
                int stripe = (py % 12) < 6 ? 0x4361EE : 0x3F37C9; int shadow = (pdx > 10) ? 2 : 1;
                uint32_t r = ((stripe >> 16) & 0xFF) / shadow; uint32_t g = ((stripe >> 8) & 0xFF) / shadow; uint32_t b = (stripe & 0xFF) / shadow;
                color = (r << 16) | (g << 8) | b;
            }
            int ring_eq = pdx*2 + pdy*6;
            if (ring_eq > -60 && ring_eq < 60 && pdist2 < 4500 && pdist2 > 1800) color = 0x7209B7;
            back_buffer[(size_t)py * width + (size_t)px] = color;
        }
    }
    gfx_mark_dirty(x, y, w, h);
...
void gfx_draw_landscape_synthwave(int x, int y, int w, int h) {
    int horizon = y + (h * 55) / 100; int sun_cx = x + w / 2; int sun_cy = horizon - (h / 6);
    for (int py = y; py < y + h; py++) {
        int rel_y = py - y;
        for (int px = x; px < x + w; px++) {
            uint32_t color = 0;
            if (py < horizon) {
                int t = (rel_y * 255) / (horizon - y);
                uint32_t r = 0x10 + ((0xFF - 0x10) * t) / 255; uint32_t g = 0x00; uint32_t b = 0x28 + ((0x70 - 0x28) * t) / 255;
                color = (r << 16) | (g << 8) | b;
                ...
                if (ground_y == 3 || ground_y == 10 || ground_y == 22 || ground_y == 40 || ground_y == 65 || ground_y == 100 || ground_y == 145 || ground_y == 200) color = 0xF72585;
                int dx = px - sun_cx;
                if (ground_y > 0) {
