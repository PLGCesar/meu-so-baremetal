...
void vfs_init(void) {
    fat32_init();
    serial_write("[VFS] VFS Conectado com Suporte a Embedded Assets de bmp.txt!\n");
}

void vfs_list(char* out_buf, size_t max_len) {
    (void)max_len;
    out_buf[0] = '\0';
    kstrcpy(out_buf, "app.elf  animacao.bgif  icons.cfg  ");
...
const uint8_t* vfs_read(const char* filename, size_t* out_size) {
    ...
    // 1. Tenta ler primeiro do disco rigido FAT32 (permite sobrescrever ícones em tempo de execução)
    const uint8_t* disk_data = fat32_read_file(clean_name, out_size);
    if (disk_data) return disk_data;

    // 2. Tenta ler de executaveis especiais
