...
// PROCESSADOR DE COMANDOS DO SHELL COM VFS DISK
void process_shell_command(void) {
    for (int i = 0; i < 128; i++) shell_output[i] = '\0';

    if (kstrcmp(input_buffer, "help") == 0) {
        const char* msg = "COMANDOS: LS, CAT <ARQ>, WRITE <ARQ> <MSG>, PANIC, CLEAR";
        ...
        // Formato: write arquivo.txt texto
        char filename[32];
        int f_idx = 0;
        int i = 6;
        ...
        if (vfs_write_file(filename, text)) {
            ...
            const char* msg = "ERRO AO GRAVAR NO DISCO!";
            for (int j = 0; msg[j] != '\0'; j++) shell_output[j] = msg[j];
        }
        ...
        const char* msg = "COMANDO DESCONHECIDO! DIGITE 'HELP' OU 'LS'.";
        for (int i = 0; msg[i] != '\0'; i++) shell_output[i] = msg[i];
    }
    input_index = 0;
    input_buffer[0] = '\0';
...
void render_gui(void) {
    ...
    if (win_open) {
        ...
        if (active_app == 0) gfx_draw_string("JANELA: TERMINAL SHELL VFS (DISCO .IMG)", win_x + 15, win_y + 11, COLOR_WHITE);
        ...
        if (active_app == 0) {
            ...
            gfx_draw_string("SAIDA DO DISCO VFS:", win_x + 50, win_y + 180, COLOR_BLUE);
    ...
    if (start_menu_open) {
        ...
        gfx_draw_string("> 1. SHELL VFS", 20, 465, active_app == 0 ? COLOR_GREEN : COLOR_WHITE);
...
void kernel_main(multiboot_info_t* mbi) {
    serial_init();
    serial_write("[LOG SERIAL] INICIALIZANDO KERNEL BARE-METAL v0.7\n");

    memory_init(mbi);
    gfx_init(mbi);
    idt_init();
    vfs_init(); // Inicializa o Sistema de Arquivos VFS do Disco .img!
