...
void fat32_init(void) {
    // Lê o Boot Sector FAT32 (Setor 0) do HD
    ata_read_sector(0, sector_buffer);
    kmemcpy(&bpb, sector_buffer, sizeof(fat32_bpb_t));

    // Se o HD estiver zerado, grava um BPB FAT32 válido!
    if (bpb.bytes_per_sector != 512) {
        serial_write("[FAT32] HD Novo! Formatando estrutura de arquivo FAT32...\n");
    ...
    serial_write("[FAT32] Volume FAT32 Montado com Sucesso!\n");
}
...
void fat32_list_files(char* out_buf, size_t max_len) {
    ...
    for (int i = 0; i < 16; i++) {
        if (entries[i].name[0] != 0x00 && (uint8_t)entries[i].name[0] != 0xE5) {
            for (int j = 0; j < 11; j++) {
                if (entries[i].name[j] != ' ' && pos < max_len - 3) {
                    out_buf[pos++] = entries[i].name[j];
                }
            }
            out_buf[pos++] = ' ';
            out_buf[pos++] = ' ';
...
const uint8_t* fat32_read_file(const char* filename, size_t* out_size) {
    ...
    for (int i = 0; i < 16; i++) {
        if (entries[i].name[0] != 0x00 && (uint8_t)entries[i].name[0] != 0xE5) {
            ...
            uint8_t* file_buf = kmalloc(512);
            ata_read_sector(file_lba, file_buf);
...
int fat32_write_file(const char* filename, const uint8_t* content, size_t size) {
    ...
    for (int i = 0; i < 16; i++) {
        if (entries[i].name[0] == 0x00 || (uint8_t)entries[i].name[0] == 0xE5) {
            ...
            // Grava a entrada no diretório raiz do FAT32
            ata_write_sector(root_lba, sector_buffer);

            // Grava o conteúdo nos setores do cluster
            uint32_t file_lba = cluster_start_sector + ((file_cluster - 2) * bpb.sectors_per_cluster);
            uint8_t data_buf[512];
            kmemset(data_buf, 0, 512);
            kmemcpy(data_buf, content, size > 512 ? 512 : size);
            ata_write_sector(file_lba, data_buf);

            serial_write("[FAT32] Arquivo gravado com sucesso no Cluster FAT32!\n");
            return 1;
        }
    }
    return 0;
