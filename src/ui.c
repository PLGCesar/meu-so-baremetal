...
typedef struct {
    int id;
    int app_type; // 0=Shell, 1=RAM, 2=IDT, 3=Galeria, 4=Musica, 5=Tarefas, 6=Paint, 7=ELF Loader
    int x, y, w, h;
    int is_open;
    int z_index;
} window_t;
...
static void process_shell_command(void) {
    ...
    if (kstrcmp(input_buffer, "help") == 0) {
        ...
        if (vfs_write_file(filename, (const uint8_t*)(input_buffer + i), kstrlen(input_buffer + i))) {
            const char* msg = "ARQUIVO GRAVADO COM SUCESSO NO DISCO VFS!";
            for (int j = 0; msg[j] != '\0'; j++) shell_output[j] = msg[j];
        }
    } else if (kstrncmp(input_buffer, "run ", 4) == 0) {
        size_t sz = 0;
        const uint8_t* elf_data = vfs_read(input_buffer + 4, &sz);
        if (elf_data) {
            if (elf_load_and_run(elf_data, sz, input_buffer + 4)) {
                const char* msg = "EXECUTAVEL ELF64 INJETADO NO ESCALONADOR COM SUCESSO!";
                for (int j = 0; msg[j] != '\0'; j++) shell_output[j] = msg[j];
            } else {
                const char* msg = "ERRO AO CARREGAR O ARQUIVO ELF64!";
                for (int j = 0; msg[j] != '\0'; j++) shell_output[j] = msg[j];
            }
        }
...
void ui_handle_mouse(void) {
    ...
    if (just_pressed) {
        ...
        if (clicked_win != -1) {
            ...
            // Controles do Player de Música (App 4)
            ...
            // Controles do Paint Studio (App 6)
            if (w->app_type == 6) {
                int py = w->y + 430;
    ...
    // ARRASTO DE JANELAS (DRAG AND DROP)
...
void draw_single_window(window_t* w) {
    ...
    if (w->app_type == 0) gfx_draw_string("JANELA: TERMINAL SHELL VFS (DISCO)", win_x + 15, win_y + 11, COLOR_WHITE);
