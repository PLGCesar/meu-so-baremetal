...
void klibc_init_cpu_features(void) {
    ...
    if ((ecx & (1 << 27)) && (ecx & (1 << 28))) avx_supported = 1;
}
...
void fast_memcpy(void* dest, const void* src, size_t n) {
    ...
    if (avx_supported && n >= 128 && ((uintptr_t)dest % 32 == 0) && ((uintptr_t)src % 32 == 0)) {
        size_t blocks_32 = n >> 5; size_t remainder = n & 31;
        __asm__ volatile (
            "1:\n\t vmovdqu (%1), %%ymm0\n\t vmovntdq %%ymm0, (%0)\n\t"
            "add $32, %0\n\t add $32, %1\n\t dec %2\n\t jnz 1b\n\t vzeroupper\n\t sfence"
            : "+r"(dest), "+r"(src), "+r"(blocks_32) : : "ymm0", "memory"
        );
        if (remainder > 0) {
    ...
    __asm__ volatile (
        ...
        : "d" (bytes) /* FIX CRÍTICO: Usa RDX para o compilador não sobreescrever RCX! */
        : "memory"
    );
}
...
void fast_memset(void* dest, uint8_t val, size_t n) {
    ...
    __asm__ volatile (
        "rep stosq\n\t mov %3, %%rcx\n\t rep stosb"
        : "+D" (dest), "+c" (qwords)
        : "a" (val64), "d" (bytes) /* FIX CRÍTICO: Usa RDX para o compilador não sobreescrever RCX! */
        : "memory"
    );
}
